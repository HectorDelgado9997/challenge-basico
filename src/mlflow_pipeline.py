...
def load_params(params_path: str = "params.yaml") -> dict[str, Any]:
    ...
    if not isinstance(params, dict):
        raise ValueError("params.yaml must contain a valid YAML dictionary.")
...
def validate_mlflow_params(params: dict[str, Any]) -> None:
    ...
    if missing_sections:
        raise ValueError(f"Missing required params sections: {sorted(missing_sections)}")
    ...
    for key in required_data_keys:
        if key not in params["data"]:
            raise ValueError(f"Missing data parameter: {key}")

    for key in required_output_keys:
        if key not in params["outputs"]:
            raise ValueError(f"Missing output parameter: {key}")

    for key in required_mlflow_keys:
        if key not in params["mlflow"]:
            raise ValueError(f"Missing MLflow parameter: {key}")
...
def run_mlflow_pipeline(params_path: str = "params.yaml") -> pd.DataFrame:
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        mlflow.log_param("text_columns", ",".join(params["pipeline"]["text_columns"]))
        mlflow.log_param(
        ...
        pys_distribution = df["pysentimiento_sentiment"].value_counts(normalize=True)
        ...
        print(f"Rows after ingestion: {rows_after_ingestion}")
        print(f"Rows after preprocessing: {rows_after_preprocessing}")
        print(f"Agreement rate: {agreement_rate:.4f}")
        print(f"Results saved to: {sentiment_results_file}")
